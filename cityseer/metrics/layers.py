...
class DataLayer:
    ...
    def assign_to_network(self, network_layer: networks.NetworkLayer, max_dist: int | float) -> None:
        ...
        if not config.QUIET_MODE:
            progress_proxy = ProgressBar(update_interval=0.25, notebook=False, total=self.data_map.count)  # type: ignore
        else:
            progress_proxy = None
    ...
    def compute_landuses(
        ...
        if not config.QUIET_MODE:
            progress_proxy = ProgressBar(update_interval=0.25, notebook=False, total=self.network_layer.network_structure.nodes.count)
        else:
            progress_proxy = None
