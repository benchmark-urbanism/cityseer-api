"""Helper functions for generating decay expression strings.

Decay expressions use a single variable `p` representing normalised progress
from 0 (source) to 1 (distance/time cutoff). The helpers below produce
expression strings that can be passed directly to the `decay_fn` parameter of
[`compute_stats`](/metrics/layers#compute-stats),
[`compute_accessibilities`](/metrics/layers#compute-accessibilities), and similar functions.
