#!/usr/bin/env python
"""
09_generate_macros.py - Generate LaTeX macros from fitted model data.
...
def load_power_exponent() -> dict | None:
    """Load power exponent analysis (optional, from 07_validate_power_exponent.py)."""
...
def load_model_comparison() -> dict | None:
    """Load model comparison results (optional, from 08_model_comparison.py)."""
...
def generate_macros() -> str:
    ...
    min_rho = min(
        gla_5km["observed_rho_h"], gla_5km["observed_rho_b"],
        gla_10km["observed_rho_h"], gla_10km["observed_rho_b"],
        gla_20km["observed_rho_h"], gla_20km["observed_rho_b"],
    )
...
def main():
    print("=" * 70)
    print("09_generate_macros.py - Generating LaTeX macros from data")
    print("=" * 70)
    ...
    for _, row in gla_df.iterrows():
        ...
        print(f"  {int(row['distance'] / 1000)}km: rho_h={row['observed_rho_h']:.3f}, rho_b={row['observed_rho_b']:.3f} (min={rho_min:.3f}), speedup={row['speedup']:.1f}x")
