...
**Low-level internals.** These are the Rust-backed structures and functions that power `cityseer`. They \
carry no stability guarantee and are subject to breaking changes from time to time. Users are encouraged to \
use the higher-level wrappers instead, which are more stable: the [`CityNetwork`](/api/network) class, or the \
[`metrics`](/metrics/networks) and [`tools`](/tools/graphs) modules. The basic information below is provided \
for those who do wish to work with the lower-level internals.
:::"""
